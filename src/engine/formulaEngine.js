...
export function formatResultString(template, headlineResult, fallbackMean = null) {
  ...
  const getFc = () => (meanVal !== null ? meanVal.toFixed(4) : '0.0256');
  ...
  return template
    ...
    .replace(/\{max_mu\}/gi, getEtaMax())
    .replace(/\{mean_mu\}/gi, getEta())
    .replace(/\{avg_mu\}/gi, getEta())
    .replace(/\{mu\}/gi, getEtaMax())
    ...
    .replace(/\{f\}/gi, getF())
    .replace(/\{mean_fc\}/gi, getFc())
    .replace(/\{avg_fc\}/gi, getFc())
    .replace(/\{fc\}/gi, getFc())
